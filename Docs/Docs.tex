...
Obrazy binarne mogą zawierać wiele niedoskonałości. W szczególności podczas tworzeniu obrazu binarnego z kolorowego \textit{(ang. tresholding)} zdarza się, że na obrazie wynikowym zostanie jakiś szum lub niechciany piksel. Operacje morfologiczne powstały aby umożliwić usuwanie tych niedoskonałości. 
\section{Erozja}
\subsection{Opis}
...
\section{Dylatacja}
\subsection{Opis}
Nakładanie \textit{(ang. dilation)} oznacza \textbf{rozszerzanie} zbioru czarnych połączonych pikseli. Świetnie sprawdza się to przy powiększaniu elementów obrazu jak i przy wypełnianiu luk. 
\subsection{Kod źródłowy algorytmu}
\section{Otwarcie}
\section{Zamknięcie}
